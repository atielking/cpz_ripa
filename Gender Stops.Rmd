---
title: "Police Stops - Gender Orientation"
...
RIPA_stops_pop_gender <- RIPA_counts_gender %>% mutate(STOP_RATE_PER_1000 = NaN) %>% mutate(STOP_RATE_TRAFFIC_PER_1000 = NaN) %>% mutate(SEARCH_RATE_PER_100_STOPS = (SEARCH_COUNT / STOP_COUNT) * 100) %>% mutate(SEARCH_RATE_CONSENT_PER_100_STOPS = (SEARCH_CONSENT_COUNT / STOP_COUNT) * 100) %>% mutate(SEARCH_RATE_PER_100_TRAFFIC_STOPS = (SEARCH_TRAFFIC_COUNT / STOP_COUNT_TRAFFIC) * 100) %>% 
...
RIPA_rates_ordered_gender <- RIPA_stops_pop_gender[,c(1, 2, 3, 12, 4, 13, 5, 14, 6, 15, 7, 16, 8, 17, 9, 18, 10, 19, 11, 20)]

RIPA_gender_final <- RIPA_rates_ordered_gender %>% mutate(IDENTITY_TYPE = 'Gender') %>% mutate(IDENTITY_SUBTYPE = GENDER) %>% select(-c(GENDER)) %>% select(1, IDENTITY_TYPE, IDENTITY_SUBTYPE, everything())

write_csv(RIPA_gender_final, path="./RIPA_gender_final.csv")
