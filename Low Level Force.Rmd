...
low_level_codes <- cds_codes_annotated %>% mutate(IS_LOW_LEVEL = ifelse(Offense.Category %in% df$codes, TRUE, FALSE)) 
...
RIPA_annotated_codes <- RIPA_force_counts %>% mutate(ROS_CUSTODIAL_WOUT_WARRANT_CDS = strsplit(as.character(ROS_CUSTODIAL_WOUT_WARRANT_CDS), ",")) %>% unnest_legacy(ROS_CUSTODIAL_WOUT_WARRANT_CDS) %>% mutate(ROS_CUSTODIAL_WOUT_WARRANT_CDS = as.numeric(ROS_CUSTODIAL_WOUT_WARRANT_CDS)) %>% filter(!is.na(ROS_CUSTODIAL_WOUT_WARRANT_CDS))

RIPA_join_cds <- left_join(RIPA_annotated_codes, low_level_codes, by=c("ROS_CUSTODIAL_WOUT_WARRANT_CDS"="Offense.Code")) %>% mutate(Offense.Level = ifelse(is.na(Offense.Level), "", Offense.Level)) %>% mutate(IS_LOW_LEVEL = ifelse(is.na(IS_LOW_LEVEL), FALSE, IS_LOW_LEVEL))

# %>% select(-c(STOP_CODE, Offense.Description)) %>% mutate(Offense.Level = ifelse(Offense.Level == "F", "Felony", "Low-Level")) %>% mutate(Offense.Level = ifelse(is.na(Offense.Level), "Low-Level", Offense.Level)) %>% arrange(AGENCY_NAME, DOJ_RECORD_ID, Offense.Level) %>% distinct(DOJ_RECORD_ID, AGENCY_NAME, PERSON_NUMBER, .keep_all = TRUE)

# RIPA_join_cds %>% group_by(AGENCY_NAME) %>% count(Offense.Level)

RIPA_force_low_level <- RIPA_join_cds %>% mutate(force_originate_misdemeanor_count = ifelse(Offense.Level == "M" & use_of_force_count == 1, 1, 0)) %>% mutate(force_originate_low_level_count = ifelse(IS_LOW_LEVEL & use_of_force_count == 1, 1, 0)) %>% mutate(gunshot_originate_misdemeanor_count = ifelse(Offense.Level == "M" & firearm_discharge_count == 1, 1, 0)) %>% mutate(gunshot_originate_low_level_count = ifelse(IS_LOW_LEVEL & firearm_discharge_count == 1, 1, 0)) %>% group_by(AGENCY_NAME) %>% summarise(
