...
# RIPA_force_counts <- RIPA_force_counts[!is.na(RIPA_force_counts$use_of_force_count), ]

RIPA_annotated_codes <- RIPA_force_counts %>% mutate(STOP_CODE = strsplit(as.character(STOP_CODE), ",")) %>% unnest_legacy(STOP_CODE) %>% mutate(STOP_CODE = as.numeric(STOP_CODE))

RIPA_annotated_codes_final <- RIPA_annotated_codes[!is.na(RIPA_annotated_codes$STOP_CODE), ]

RIPA_join_cds <- left_join(RIPA_annotated_codes_final, CDS_Codes, by=c("STOP_CODE"="CDS.Code")) %>% select(-c(STOP_CODE, Offense.Description)) %>% mutate(Offense.Level = ifelse(Offense.Level == "F", "Felony", "Low-Level"))

low_level <- RIPA_join_cds %>% group_by(DOJ_RECORD_ID) %>% summarize(IS_LOW_LEVEL = ifelse(any(Offense.Level == "Felony"), FALSE, TRUE))

#RIPA_join_cds <- RIPA_join_cds %>% select(-c(Offense.Level))

RIPA_low_level <- left_join(low_level, RIPA_join_cds, by="DOJ_RECORD_ID")

RIPA_low_level_final <- RIPA_low_level[!is.na(RIPA_low_level$IS_LOW_LEVEL), ]


RIPA_force_low_level <- RIPA_low_level_final %>% mutate(force_originate_low_level_count = ifelse(IS_LOW_LEVEL & use_of_force_count == 1, 1, 0)) %>% mutate(gunshot_originate_low_level_count = ifelse(IS_LOW_LEVEL & firearm_discharge_count == 1, 1, 0)) %>% group_by(AGENCY_NAME) %>% summarise(
...
RIPA_force %>% group_by(AGENCY_NAME) %>% count(use_of_force_count)

# RIPA_force_low_level <- RIPA_force_low_level[!is.na(RIPA_force_low_level$AGENCY_NAME), ]
